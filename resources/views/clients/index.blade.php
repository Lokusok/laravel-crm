                                            ...
                                            <tr>
                                                ...
                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                    <div class="flex items-center gap-x-6">
                                                        <a href="{{ route('users.edit', $client) }}" class="text-gray-500 transition-colors duration-200 hover:text-gray-800 active:opacity-60 focus:outline-none">
                                                            Edit
                                                        </a>

                                                        @unless ($client->is(Auth::user()))
                                                            <form
                                                                action="{{ route('users.destroy', $client) }}"
                                                                method="POST"
                        ...
                        <div class="flex items-center justify-between mt-6">
                            <a
                                ...
                                @class([
                                    ...
                                    "pointer-events-none opacity-50" => ! $clients->previousPageUrl()
                                ])
                            >
                            ...
                            <div class="items-center hidden md:flex gap-x-3">
                                @for ($page = 1; $page <= $clients->lastPage(); $page++)
                                    <a
                                        href="{{ route('users.index', ['page' => $page]) }}"
                                        @class([
                                            "px-2 py-1 text-sm text-blue-500 rounded-md bg-blue-100/60",
                                            "bg-blue-700 text-white font-bold" => $page === $clients->currentPage(),
                                        ])
                                    >
                                        {{ $page }}
                                    </a>
                            ...
                            <a
                                ...
                                @class([
                                    "flex items-center px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md gap-x-2 hover:bg-gray-100 ",
                                    "pointer-events-none opacity-50" => ! $clients->nextPageUrl()
                                ])
                            >
                                <span>
                                    Next
